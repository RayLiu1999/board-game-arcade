...
function hand(s, str) {
  s.game.model.shoupai[0] = Majiang.Shoupai.fromString(str);
}
test("four private views expose only the requesting seat’s concealed tiles", () => {
  const s = started();
...
test("riichi actions are bound to a seat and cannot be replayed", () => {
  const s = started(),
    c = s.view(0).choices.find((c) => c.kind === "discard");
  assert.throws(() => s.act(1, c.id));
  assert.throws(() => s.act(0, "invented"));
  s.act(0, c.id);
  assert.throws(() => s.act(0, c.id));
  for (let i = 0; i < 10; i++) s.step();
  assert(s.game.model.he[0]._pai.length === 1);
  s.pause();
  if (s.pending.size) {
    const id = [...s.pending.keys()][0];
    assert.throws(() => s.act(id, s.view(id).choices[0].id));
  }
  s.close();
});
test("winning hand requires yaku; dora alone is not a yaku", () => {
  const s = started();
...
test("riichi choices require a closed tenpai hand and enough points", () => {
  const s = started();
  s.game._diyizimo = false;
...
test("chi only available from the player on the left; pon and kan exposed legally", () => {
  const s = started();
...
test("furiten suppresses ron while self draw remains available", () => {
  const s = started();
  s.game.model.lunban = 0;
  s.game._dapai = "z1";
...
test("a single-hand game reaches scored results and a final ranking", () => {
  const s = started();
  let results = 0;
  ...
  assert(s.done);
  assert(results >= 4);
  assert.equal(s.view(0).result.rank.length, 4);
  assert.equal(s.view(0).result.log, undefined);
  assert.equal(
    s.view(0).result.scores.reduce((a, b) => a + b, 0),
    100000,
  );
  s.close();
});
test("AI opponents progress without seeing another player’s private messages", () => {
...
test("seven pairs tsumo pays correct fu, han, dealer points and final scores", () => {
  const s = started();
  s.game._diyizimo = false;
  s.game.model.shan._baopai = ["z7"];
  ...
  for (const id of [...s.pending.keys()]) s.act(id, s.view(id).choices[0].id);
  for (let i = 0; i < 10; i++) s.step();
  assert(s.done);
  assert.deepEqual(s.view(0).result.scores, [29800, 23400, 23400, 23400]);
  s.close();
});
