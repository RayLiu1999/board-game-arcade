...
├─ websocket-server.ts
├─ room-store.ts
├─ postgres-room-store.ts
├─ room-store-factory.ts
├─ room-security.ts
...
目前已有規則契約、協定邊界、WebSocket 整合與瀏覽器 E2E 測試，並已支援可注入的 deterministic AI seed、可重現的初始局面走訪、AI 合法走法驗證、property-based 合法路徑與 reachable random positions 測試；後續可增加：
...
5. **P3-1 帳號、戰績、配對與對戰聊天室**：需要 user／match／rating schema、驗證、封鎖、聊天室權限與反作弊邊界。
6. **P3-2 觀戰、比賽與對局時鐘**：需要觀戰者權限、事件廣播、斷線重連、觀戰聊天室邊界與審計紀錄。
